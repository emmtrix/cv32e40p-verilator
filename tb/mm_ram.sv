...
`endif
 #(
  ...
  (
    ...
    input logic [31:0]                   data_core_id_i,
    ...
    localparam int                        EU_BAR_IRQ_ID  = 30;
    ...
    localparam int                        MMADDR_EU_BASE    = 32'h1500_2000;
    localparam int                        MMADDR_EU_WAIT_BARRIER = MMADDR_EU_BASE + 32'h0000;
    localparam int                        MMADDR_EU_SET_BARRIER  = MMADDR_EU_BASE + 32'h0040;
    localparam int                        MMADDR_EU_EVT_WAIT     = MMADDR_EU_BASE + 32'h0080;
    localparam int                        MMADDR_EU_GPEVT_CLEAR  = MMADDR_EU_BASE + 32'h0084;
    ...
    enum logic [2:0]{RAM, MM, RND_STALL, ERR, RND_NUM, TICKS, EU_EVT} select_rdata_d, select_rdata_q;
...
`endif
    ...
    always_comb begin
        ...
        rnd_stall_we        = '0;
        rnd_num_req         = '0;
        eu_bar_notify_valid = '0;
        eu_bar_notify_wdata = '0;
        eu_bar_setup_valid  = '0;
...
`endif // ifndef VERILATOR
                ...
                end else if (data_addr_i == MMADDR_EU_WAIT_BARRIER) begin
                    eu_bar_notify_valid = 1'b1;
                    eu_bar_notify_wdata = data_wdata_i;
                end else if (data_addr_i == MMADDR_EU_SET_BARRIER) begin
                    eu_bar_setup_valid = 1'b1;
                    eu_bar_setup_wdata = data_wdata_i;
                end else if (data_addr_i == MMADDR_EU_GPEVT_CLEAR) begin
                    eu_evt_clear_valid = 1'b1;
                    eu_evt_clear_wdata = data_wdata_i;
     ...
     (data_req_i && data_we_i |-> data_addr_i < 2 ** RAM_ADDR_WIDTH
      || ( (data_addr_i >= dm_halt_addr_i) &&
         ...
         || data_addr_i == MMADDR_TICKS
                 || data_addr_i == MMADDR_EU_WAIT_BARRIER
                 || data_addr_i == MMADDR_EU_SET_BARRIER
                 || data_addr_i == MMADDR_EU_GPEVT_CLEAR
