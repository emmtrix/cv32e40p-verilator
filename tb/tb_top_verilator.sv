module tb_top_verilator #(
    ...
    initial begin : check_num_cores
        if (NUM_CORES != 1) begin
            $fatal(1, "[TB] This testbench is configured for NUM_CORES=1");
        end
    end
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (tests_passed_o) begin
                $display("[TB] SINGLE CORE TESTS PASSED");
                $finish;
            end
            if (tests_failed_o) begin
                $fatal(1, "[TB] SINGLE CORE TESTS FAILED");
            end
            if (exit_valid) begin
                if (exit_value == 0) begin
                    $display("[TB] SINGLE CORE EXIT SUCCESS");
                    $finish;
                end else begin
                    $fatal(1, "[TB] SINGLE CORE EXIT FAILURE: %0d", exit_value);
                end
            end
        end
    end
